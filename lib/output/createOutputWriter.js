...
module.exports = function (options) {
  ...
  options.isEmptyObject = function (O){
      for (var x in O){
        return false;
      }
      return true;
  ...
  return function writeOutput (newAssets, next) {
    ...
    mkdirp(options.path, function (err) {
      ...
      fs.readFile(outputPath, 'utf8', function (err, data) {
        ...
        var _data = overwrite ? '{}' : data || '{}';

        var oldAssets;
        try {
          oldAssets = JSON.parse(_data);
          
        } catch (err) {
          oldAssets = {};
        }
        
        if(data != undefined && options.isEmptyObject(oldAssets)){
            return next(error("'"+outputPath+"' parse error",err));            
        }
        
        var assets = merge({}, oldAssets, newAssets);
        //var output = options.processOutput(assets); 
        var output = JSON.stringify(assets, null, options.prettyPrint ? 4 : null); 
        if (output !== _data) {
